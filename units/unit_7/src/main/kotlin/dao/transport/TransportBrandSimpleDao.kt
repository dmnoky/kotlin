...
class TransportBrandSimpleDao private constructor() : TransportBrandDao {
    companion object {
        ...
        private val TRANSPORT_MODEL_BRAND_SQL_DELETE = { id: UUID -> "DELETE FROM $TABLE_NAME_TRANSPORT_MODEL_BRAND WHERE brand_id='$id'" }
    }
    ...
    override fun delete(connection: Connection, brand: TransportBrand): Boolean {
        var sql = TRANSPORT_MODEL_BRAND_SQL_DELETE(brand.id)
        println(sql)
        connection.createStatement().use {
            it.execute(sql) //удаление из интерсекшен таблицы
            sql = TRANSPORT_BRAND_SQL_DELETE(brand.id)
            println(sql)
