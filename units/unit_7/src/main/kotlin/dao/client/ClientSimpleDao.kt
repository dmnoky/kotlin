...
class ClientSimpleDao private constructor() : ClientDao {
    companion object {
        ...
        private const val CLIENT_SQL_QUERY = """
            SELECT c.id client_id, c.version client_version, c.delete_at client_delete_at, c.city client_city,
                c.last_name client_last_name, c.first_name client_first_name, c.middle_name client_middle_name
            FROM $TABLE_NAME_CLIENT c
        """
        private const val CLIENT_SQL_QUERY_ALL = """
            $CLIENT_SQL_QUERY
        ...
        """
        //"DELETE FROM $TABLE_NAME_CLIENT WHERE id='$clientId'"
        private const val CLIENT_SQL_DELETE = """
            UPDATE $TABLE_NAME_CLIENT SET version=?, delete_at=now()
            WHERE id=? AND version=?"""
        ...
        private val CLIENT_SQL_SELECT_BY_TRANSPORT_ID = { transportId: UUID ->
            "$CLIENT_SQL_QUERY LEFT JOIN $TABLE_NAME_CLIENT_TRANSPORT ct ON ct.client_id = c.id WHERE c.delete_at is null AND ct.transport_id='$transportId'"
        }
        private const val CLIENT_SQL_SELECT_FIO_BY_BRAND = """
                ...
                JOIN $TABLE_NAME_TRANSPORT_BRAND t_b ON t_b.id = t_m_b.brand_id AND t_b.brand=?
                WHERE c.delete_at is null
            """
    }
    ...
    override fun findAll(connection: Connection): LinkedHashSet<Client> {
        println(CLIENT_SQL_QUERY_ALL)
        connection.createStatement().use { st ->
            st.executeQuery(CLIENT_SQL_QUERY_ALL).use { rs ->
    ...
    override fun getFIOByTransportBrand(connection: Connection, brand: String): LinkedList<String> {
        println(CLIENT_SQL_SELECT_FIO_BY_BRAND)
        connection.prepareStatement(CLIENT_SQL_SELECT_FIO_BY_BRAND).use { st ->
    ...
    override fun delete(connection: Connection, client: Client): Boolean {
        println(CLIENT_SQL_DELETE)
        connection.prepareStatement(CLIENT_SQL_DELETE).use {
            it.setLong(1, client.version + 1)
            //WHERE
            it.setObject(2, client.id)
            it.setLong(3, client.version)
            it.execute()
            if (it.updateCount == 1) return true
            else throw OptimisticLockException("Запись ${client.id} изменена другим пользователем!")
        }
    }
    ...
    override fun update(connection: Connection, client: Client): Boolean {
        ...
        connection.prepareStatement(CLIENT_SQL_UPDATE).use {
            ...
            it.setString(5, client.city)
            //WHERE
            it.setObject(6, client.id)
            it.setLong(7, client.version)
            it.execute()
            if (it.updateCount == 1) return true
            else throw OptimisticLockException("Запись ${client.id} изменена другим пользователем!")
        }
