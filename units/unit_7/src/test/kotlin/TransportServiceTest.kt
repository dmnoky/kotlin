...
class TransportServiceTest {
    ...
    fun notFoundById_then_add_then_findById_then_uniqueErr() {
        val transport = Transport.buildNewTransport("413213211", transportModel.findByName("X3"))
    ...
    @Test
    fun add_then_delete() {
        val transport = Transport.buildNewTransport("2413213211", transportModel.findByName("X3"))
        assertThrows<EntityNotFoundException> { transportService.findById(transport.id) }
        transportService.add(transport)
        assertEquals(transportService.findById(transport.id).id, transport.id)
    ...
    fun findByClientId() {
        ...
        assertEquals(transportByClientId.size, 3)
